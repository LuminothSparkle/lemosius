unsigned line_off(const char* ini, const token_anotada& t) {
   return 1 + std::count(ini, t.original.data(), '\n');
}

unsigned col_off(const char* ini, const token_anotada& t) {
   return 1 + (t.original.data() - std::find(std::reverse_iterator(t.original.data()), std::reverse_iterator(ini), '\n').base( ));
}

std::string_view view_off(const token_anotada& t, int len, const char* fin) {
   return { t.original.data(), std::size_t(std::find(t.original.data(), (fin - t.original.data() <= len ? fin : t.original.data() + len), '\n') - t.original.data()) };
}

void error_report(std::ostream& os, const char* ini, const char* fin, const std::pair<token_anotada, const char*>& e) {
