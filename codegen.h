...
void write_program( const program_resources& pr, const resolution_table& tbl, bool main_file, std::ostream& os ) {
   ...
   if (main_file) {
      os << "int main( ) {" << get_identifier_declaration(*pr.function_overloads.at("main").at(0).declaration) << "( ); }\n";
   }
}
